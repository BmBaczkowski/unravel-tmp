...
parameters{
    ...
    matrix[S_olditems, K1] u_beta_rho;
    //-----------
}
transformed parameters {
    ...
    matrix[K1, S_olditems] u_beta_rho_transposed;
    u_beta_rho_transposed = transpose(u_beta_rho);
    // matrix[K1, K1] Omega;
    ...
    for (i in 1:N_olditems){
        beta_rho[i] = X_design_olditems[i, :] * (
            // mu_beta_rho 
            + u_beta_rho_transposed[:, sub_id_olditems[i]]
...
model {
    ...
    target += normal_lpdf(to_vector(u_beta_rho) | to_vector(f), gp_sigma);
    //--------------------
