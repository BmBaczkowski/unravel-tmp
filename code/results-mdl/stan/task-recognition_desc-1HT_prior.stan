...
generated quantities {
    ...
    */
    ...
    vector<lower=0.01, upper=0.5>[K1] tau_w_beta_rho;
    vector<lower=0.01, upper=0.5>[K2] tau_w_beta_gamma;
    // adjustments
    ...
    */
    ...
    mu_beta_rho[1] = normal_rng(-0.5, 1);
    mu_beta_gamma[1] = normal_rng(-0.8, 1);
    // remaining beta parameters
    for (i in 2:K1){
        mu_beta_rho[i] = normal_rng(0, 1);
    }
    for (i in 2:K2){
        mu_beta_gamma[i] = normal_rng(0, 1);
    }
    // random effects (participants)
    for (i in 1:K1){
        tau_u_beta_rho[i] = gamma_rng(5, 10);
    }
    for (i in 1:K2){
        tau_u_beta_gamma[i] = gamma_rng(5, 10);
    }
    Lcorr_beta_rho = lkj_corr_cholesky_rng(K1, 2);
    Lcorr_beta_gamma = lkj_corr_cholesky_rng(K2, 2);
    for (i in 1:K1) {
        for (j in 1:S_olditems){
            z_u_beta_rho[i,j] = std_normal_rng();
        }
    }
    for (i in 1:K2) {
        for (j in 1:S_newitems) {
            z_u_beta_gamma[i,j] = std_normal_rng();
        }
    }
    // random effects (studies)
    for (i in 1:K1){
        tau_w_beta_rho[i] = uniform_rng(0.01, 0.5);
    }
    for (i in 1:K2){
        tau_w_beta_gamma[i] = uniform_rng(0.01, 0.5);
    }
    for (i in 1:K1) {
        for (j in 1:St_olditems){
            z_w_beta_rho[i,j] = std_normal_rng();
        }
    }
    for (i in 1:K2) {
        for (j in 1:St_newitems){
            z_w_beta_gamma[i,j] = std_normal_rng();
        }
    }
    // fixed effects (semantic category)
    gm_c_beta_rho = normal_rng(0, 1);
    gm_c_beta_gamma = normal_rng(0, 1);
    /* 
