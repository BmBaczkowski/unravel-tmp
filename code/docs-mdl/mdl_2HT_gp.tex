...
\begin{tabular}{ m{8cm} m{9cm}}
  ...
  \begin{tikzpicture}[node distance=2.5cm, auto, >=latex']
    ...
    %% GP
    \node [continuousLatent] (f) [above right = 10mm and -2mm of beta_i_j] {$f$};
    \node [continuousLatent] (sigmagp) [above left = 9.8mm and 0.8mm of beta_i_j] {$\sigma_{gp}$};
  ...
  % %% Draw edges
  \path 
    (f) edge [connect] (beta_i_j)
    (sigmagp) edge [connect] (beta_i_j)
...
\begin{array}{rcl}
    ...
    \tau^{gp} & \sim & invGamma(5, 5) \\
    k(x | \tau^{gp}, \ell) & = & \tau^{gp} \cdot \left( 1 + \frac{\sqrt{3} \|x - x'\|}{\ell} \right) \exp\left(-\frac{\sqrt{3} \|x - x'\|}{\ell}\right) \\ 
    f & \sim & MvNormal(\mathbf{0}, k(x | \tau^{gp}, \ell)) \\ 
