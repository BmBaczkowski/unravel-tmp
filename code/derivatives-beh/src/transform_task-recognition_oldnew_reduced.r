...
preproc_df <- function(df, study) {
    df <- df %>%
        ...
        summarize(
            n_old = sum(response == "old", na.rm = TRUE), # Count "old" responses
            n_total = sum(!is.na(response)), # Count total responses
            .groups = 'drop' # Drop grouping structure
        ) %>%
        mutate(
            study_id = str_extract(study, "\\d+") # Extract study ID from study string
        ) %>%
        rename(
            first_presentation = item_first_presentation, # Rename column
            status = item_status # Rename column
        ) %>%
        select(study_id, everything()) # Ensure study_id is first column
...
main <- function(source_sub_dir, target_sub_dir) {
    ...
    target_tsv_file <- gsub("_beh.tsv", "_desc-reduced_beh.tsv", source_tsv_file)
